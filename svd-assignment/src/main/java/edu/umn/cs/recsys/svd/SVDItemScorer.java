...
public class SVDItemScorer extends AbstractItemScorer {
    ...
    public void score(long user, @Nonnull MutableSparseVector scores) {
    	...
    	RealMatrix userMatrix=model.getUserVector(user);
    	RealMatrix weights=model.getFeatureWeights();
    	
    	MutableSparseVector userRatings=getUserRatingVector(user).mutableCopy();
    	MutableSparseVector baselines=MutableSparseVector.create(userRatings.keySet());
    	...
    	if(null!=userMatrix) {
    		for (VectorEntry e: scores.fast(VectorEntry.State.EITHER)) {
    			long item = e.getKey();
    			/*if(baselines.containsKey(item)==false) {
            	scores.unset(item);
        		continue;
        	}*/
    			// TODO Set the scores
    			RealMatrix itemMatrix=model.getItemVector(item);
    			RealMatrix vt=itemMatrix.transpose();

    			double score=userMatrix.multiply(weights).multiply(vt).getEntry(0,0);
    			double baselineScore=baselineScorer.score(user, item);
    			double finalScore=score+baselineScore;
