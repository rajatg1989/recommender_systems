...
public class SVDModelBuilder implements Provider<SVDModel> {
    ...
    public SVDModel get() {
        ...
        RealMatrix umat=svd.getU().getSubMatrix(0, userMapping.size()-1, 0, featureCount-1);
        RealMatrix imat=svd.getV().getSubMatrix(0, itemMapping.size()-1, 0, featureCount-1);
    ...
    private RealMatrix createRatingMatrix(IdIndexMapping userMapping, IdIndexMapping itemMapping) {
        ...
        try {
            for (UserHistory<Event> user: users) {
                ...
                for (VectorEntry ve:ratings.fast()) {
                	long itemId=ve.getKey();
                	double rating=ve.getValue();
                	double normalisedRating=rating-baselines.get(itemId);
                	int itemIndex=itemMapping.getIndex(itemId);
                	matrix.setEntry(u, itemIndex, normalisedRating);
                }
            }
        } finally {
            users.close();
